"""MCP HTTP/SSE server exposing ManimCE v0.20.1 API reference as tools.

Runs as a standalone Cloud Run service. Agents connect via SSE transport
instead of spawning a subprocess — fully concurrent-safe for any number
of simultaneous agent pipelines.
"""
...
async def handle_sse(request: Request):
    """SSE endpoint — each agent opens a persistent connection here."""
...
async def health(request: Request):
    return JSONResponse({"status": "ok", "service": "manim-mcp-server"})
